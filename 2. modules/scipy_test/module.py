...
def do_exam2():
    ...
    # plt.figure(num=1. Basic, dpi=100, facecolor="white")
    plt.figure()
    plt.plot(t,y,"r")
    plt.xlim(0, 0.05)
    plt.xlabel("time($sec$")
...
#
# def FilteredSignal(signal, fs, cutoff):
#     B, A = butter(1. Basic, cutoff / (fs / 2. modules), btype="low")
#     filtered_signal = filtfilt(B, A, signal, axis=0)
#     return filtered_signal
#
# fs = 10000.
# T = .1. Basic
# time = np.arange(0., T, 1. Basic / fs)
# frequency = 1000
# noise = np.random.normal(0, 1. Basic, int(fs/10))
# signal = np.sin(2. modules * np.pi * frequency * time) + 0.2. modules * noise
# analyticSignal = hilbert(signal)
# amplitudeEnvelope = np.abs(analyticSignal)
#
# cutoff = 1000
# filteredSignal = FilteredSignal(amplitudeEnvelope, fs, cutoff=cutoff)
# fig, ax = plt.subplots(1. Basic, 1. Basic)
# ax.plot(time, signal)
# ax.plot(time, filteredSignal)
# ax.set_xlabel("Tiempo")
# ax.set_ylabel("Amplitud")
